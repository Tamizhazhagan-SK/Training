...
## 2. What is Helm & Helm Charts?
* **Helm:** The package manager for Kubernetes (comparable to `apt` for Ubuntu or `npm` for Node.js). It simplifies finding, sharing, and using software built for Kubernetes.
* **Helm Chart:** A collection of files that pack all the required Kubernetes resources (Deployments, Services, CRDs, RBAC rules, etc.) into a single, version-controlled package.
* **Values (`values.yaml`):** Configuration parameters inside a chart that allow you to customize settings (such as resource limits, node selectors, or storage directories) without manually editing raw YAML files.
* **Rook & Helm Integration:** Rook and Ceph are predominantly deployed using Helm charts. You deploy the `rook-ceph` operator chart to initialize the operator, which then listens for commands to build and manage the Ceph cluster.

---

## 3. How Rook Works with Kubernetes
...
## 4. Core Ceph Components in a Kubernetes Cluster
